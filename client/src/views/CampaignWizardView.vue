...
/* ── Step 2 — review (read-only preview) ──────────────── */
// creating state managed here so the button shows a spinner
...
async function saveEntity() {
  if (!entityForm.name.trim()) {
    ui.showToast('Entity name is required', '', '✕')
    return
  }
  ...
  <div class="wiz-page">
    <div class="wiz-wrap">
      ...
      <template v-if="step === 0">
        ...
        <p class="wiz-sub">You can change these details any time from the campaign settings.</p>
        ...
        <div class="wiz-btn-row">
          <button class="wiz-btn ghost" @click="router.push('/home')">← Cancel</button>
      ...
      <template v-if="step === 2">
        ...
        <p class="wiz-sub">These will be applied when your campaign is created. You can customise everything after setup.</p>
      ...
      <template v-if="step === 3">
        ...
        <p class="wiz-sub">Kick off your world with an NPC, location, faction or quest. You can always skip and add these from the sidebar.</p>
        ...
        <div v-if="importTab === 1">
          <div class="wiz-warning">
            ...
            <span>Paste import is coming soon — use the guided wizard or upload a file for now.</span>
          </div>
        </div>
...
@media (min-width: 480px) { .wiz-step-label { display: inline; } }
...
.wiz-field textarea:focus {
  border-color: rgba(168,192,128,.5);
}
...
.wiz-field select option { background: var(--bg, #2a2d28); }
...
.wiz-btn.ghost:hover:not(:disabled) { color: rgba(255,255,255,.6); background: transparent; }
...
.wiz-upload-zone:hover { border-color: rgba(168,192,128,.3); }
...
@media (max-width: 600px) {
  .wiz-page { padding: 24px 16px 48px; }
  .wiz-field-row { grid-template-columns: 1fr; }
  .wiz-preview-grid { grid-template-columns: 1fr; }
  .wiz-system-grid { grid-template-columns: repeat(auto-fill, minmax(120px, 1fr)); }
}
</style>
