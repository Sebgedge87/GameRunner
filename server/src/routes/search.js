...
router.get('/', requireAuth, (req, res) => {
  ...
  const quests = db.prepare("SELECT id, title, status as subtitle, 'quest' as type FROM quests WHERE title LIKE ? LIMIT 5").all(like);
  const npcs = db.prepare("SELECT id, title, role as subtitle, 'npc' as type FROM npcs WHERE title LIKE ? OR role LIKE ? LIMIT 5").all(like, like);
  const locations = db.prepare("SELECT id, title, status as subtitle, 'location' as type FROM locations WHERE title LIKE ? LIMIT 5").all(like);
  const hooks = db.prepare("SELECT id, title, type as subtitle, 'hook' as type FROM hooks WHERE title LIKE ? LIMIT 5").all(like);
