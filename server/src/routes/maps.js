...
router.get('/', requireAuth, (req, res) => {
  ...
  const maps = campId
    ? db.prepare('SELECT * FROM maps WHERE (campaign_id = ? OR campaign_id IS NULL) ORDER BY created_at DESC').all(campId)
    : db.prepare('SELECT * FROM maps ORDER BY created_at DESC').all();
  res.json({ maps });
});
